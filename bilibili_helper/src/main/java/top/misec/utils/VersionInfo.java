...
public class VersionInfo {
    ...
    public static void printVersionInfo() {
        ...
        log.info("-----版本信息-----");
        log.info("当前版本: %s", releaseVersion);
        try {
            log.info("最新版本为: %s", jsonObject.get("tag_name").getAsString().replaceAll("v", ""));
            log.info("-----最新版本更新内容-----\n%s", jsonObject.get("body").getAsString().replaceAll("\"", ""));
            log.info("最近更新时间: %s", HelpUtil.utcTime(jsonObject.get("created_at").getAsString()));
            log.info("项目备份地址: %s", projectRepo);
            log.info("-----版本信息-----\n");
        } catch (Exception e) {
