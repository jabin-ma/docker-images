...
public class OftenApi {
    ...
    public static Double getCoinBalance() {
        ...
        if (responseCode == 0) {
            ...
            log.warn("请求硬币余额接口错误，请稍后重试。错误请求信息：%s", responseJson);
            return 0.0;
        }
    }
    ...
    public static void getVipPrivilege(int type) {
        ...
        if (responseCode == 0) {
            ...
            log.warn("领取年度大会员每月赠送的B币券/大会员福利失败，原因: %s", jsonObject.get("message").getAsString());
        }
    }
    ...
    public static String queryUserNameByUid(String uid) {
        ...
        if (jsonObject.get("code").getAsInt() == 0) {
            ...
            log.info("查询充电对象的用户名失败，原因：%s", jsonObject);
        }
        return userName;
    }
