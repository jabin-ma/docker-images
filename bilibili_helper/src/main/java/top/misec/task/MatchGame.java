...
public class MatchGame implements Task {
    ...
    public boolean run(BilibiliRuntime bilibiliRuntime) {
        return bilibiliRuntime.runWithL(log -> {
            ...
            if (currentCoin < ConfigLoader.helperConfig.getTaskConfig().getMinimumNumberOfCoins()) {
                log.info("%s个硬币都没有，参加什么预测呢?任务结束", ConfigLoader.helperConfig.getTaskConfig().getMinimumNumberOfCoins());
                return true;
            }
            ...
            if (resultJson.get("code").getAsInt() == 0) {
                ...
                if (list != null) {
                    ...
                    for (JsonElement listinfo : list) {
                        ...
                        if (currentCoin < ConfigLoader.helperConfig.getTaskConfig().getMinimumNumberOfCoins()) {
                            log.info("仅剩%s个硬币，低于最低保留硬币数量，后续预测不再执行", currentCoin);
                            break;
                        }
                        ...
                        log.info("%s %s:%s", seasonName, contestName, questionTitle);
                        ...
                        log.info("当前赔率为:  %s:%s", teamA.get("odds").getAsDouble(), teamB.get("odds").getAsDouble());
    ...
    private void doPrediction(int oid, int main_id, int detail_id, int count, BilibiliRuntime.Log log) {
