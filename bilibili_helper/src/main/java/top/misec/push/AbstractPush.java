...
public abstract class AbstractPush implements Push, RetryListener {
    ...
    private PushResult push2Target(String url, String pushContent) {
        try {
            ...
            log.info("推送结果：%s", jsonObject.toString());
            return PushResult.success();
        } catch (RetryException e) {
            log.error("重试最终失败：%s", this.getClass().getSimpleName(), e);
        } catch (ExecutionException e) {
            log.error("重试中断：%s", this.getClass().getSimpleName(), e);
        }
    ...
    public <V> void onRetry(Attempt<V> attempt) {
        if (attempt.hasException()) {
            log.error("推送失败 已尝试：%s次", attempt.getAttemptNumber(), attempt.getExceptionCause());
            return;
        }
        if (attempt.hasResult()) {
            log.error("推送返回失败：%s 已尝试：%s次", attempt.getResult(), attempt.getAttemptNumber());
        }
    }
